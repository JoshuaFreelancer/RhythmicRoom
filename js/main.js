            ...
            data.tracks.track.forEach(track => {
                ...
                fetch(`${apiUrl}?method=track.getInfo&api_key=${apiKey}&artist=${track.artist.name}&track=${track.name}&format=json`)
                    ...
                    .then(trackInfo => {
                        ...
                        image.src = trackInfo.track.album.image[2]['#text'] || 'placeholder.jpg';
...
async function loadMusicGenres() {
    ...
    try {
        const response = await fetch(`${apiUrl}?method=tag.getTopTags&api_key=${apiKey}&format=json`);
        ...
        console.error('Error al obtener datos de la API (Géneros musicales):', error);
    }
}
...
async function searchArtists(searchTerm) {
    ...
    try {
        const response = await fetch(`${apiUrl}?method=artist.search&artist=${encodeURIComponent(searchTerm)}&api_key=${apiKey}&format=json`);
        const data = await response.json();
        ...
        const artistImages = await Promise.all(artists.map(async artist => {
            try {
                const artistInfoResponse = await fetch(`${apiUrl}?method=artist.getinfo&artist=${encodeURIComponent(artist.name)}&api_key=${apiKey}&format=json`);
                ...
                if (artistImages && artistImages.length > 0) {
                    ...
                    console.warn('No se encontraron imágenes para el artista:', artist.name);
        ...
        displayResults(artists, 'artistsResults', 8, 'Resultados de Artistas', artistImages);
    } catch (error) {
...
async function searchAlbums(searchTerm) {
    ...
    try {
        const response = await fetch(`${apiUrl}?method=album.search&album=${encodeURIComponent(searchTerm)}&api_key=${apiKey}&format=json`);
        const data = await response.json();
        const albums = data.results.albummatches.album;

        // Obtener imágenes para álbumes
        const albumImages = await Promise.all(albums.map(async album => {
            const albumInfo = await fetch(`${apiUrl}?method=album.getinfo&artist=${encodeURIComponent(album.artist)}&album=${encodeURIComponent(album.name)}&api_key=${apiKey}&format=json`);
            const albumData = await albumInfo.json();
            return albumData.album.image[2]['#text']; // Usamos la tercera imagen (de tamaño medio)
        }));

        displayResults(albums, 'albumsResults', 8, 'Resultados de Álbumes', albumImages);
    } catch (error) {
...
async function searchTracks(searchTerm) {
    ...
    try {
        const response = await fetch(`${apiUrl}?method=track.search&track=${encodeURIComponent(searchTerm)}&api_key=${apiKey}&format=json`);
        const data = await response.json();
        const tracks = data.results.trackmatches.track;

        // Obtener imágenes para pistas
        const trackImages = await Promise.all(tracks.map(async track => {
            try {
                const trackInfoResponse = await fetch(`${apiUrl}?method=track.getinfo&artist=${encodeURIComponent(track.artist)}&track=${encodeURIComponent(track.name)}&api_key=${apiKey}&format=json`);
        ...
        displayResults(tracks, 'tracksResults', 12, 'Resultados de Pistas', trackImages);
    } catch (error) {
